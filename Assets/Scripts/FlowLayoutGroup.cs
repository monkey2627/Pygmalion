...
{
    [Tooltip("0 表示用父 RectTransform 宽度")]
    ...
    public void Refresh()
    {
        float usableWidth = maxWidth > 0 ? maxWidth : rt.rect.width;

        /* ---------- 第一遍：流式排布，同时收集行信息 ---------- */
        lines.Clear();
        Line curLine = null;
        float curX = 0f, curY = 0f, lineH = 0f;
        ...
        for (int i = 0; i < rt.childCount; i++)
        {
            ...
            {
                curY -= lineH * lineHeightScale + lineSpacing;
                curX = 0f;
            }
        ...
        rt.SetSizeWithCurrentAnchors(RectTransform.Axis.Vertical,
                                     Mathf.Abs(curY) + lineH * lineHeightScale);
    }
